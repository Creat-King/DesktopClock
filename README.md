...
> 一个轻量级 Windows 桌面时钟，透明显示、点击穿透、不遮挡任何界面内容。
...
- **20KB executable** — Smaller than most favicon files.
...
一个真正「不挡手」的 Windows 桌面时钟。时间始终悬浮在所有窗口之上，但鼠标可以完全穿透它操作下方的界面。不出现在 Alt+Tab、不出现在任务栏、不干扰任何操作。
...
C:\Windows\Microsoft.NET\Framework64\v4.0.30319\csc.exe ^
  ...
  Program.cs ClockWindow.cs TrayManager.cs Win32.cs Settings.cs
```
...
├── ClockWindow.cs      # 透明置顶窗口 + 时钟显示 + 穿透/拖动逻辑
├── TrayManager.cs      # 系统托盘图标 + 右键菜单 + 热键
├── Win32.cs            # P/Invoke：窗口样式、热键注册
├── Settings.cs         # 配置模型 + 加载/保存
├── build.bat           # 一键编译脚本
├── DesktopClock.exe    # 编译好的可执行文件（20KB）
└── README.md
```
